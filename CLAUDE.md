...
- **테이스팅 노트 상세** `/whisky/[id]`: 템플릿 레이아웃(정보=종류·증류소·도수·가격·상점 / 감각=색·향(Nose)·맛(Palate)·향(Aroma)·맛(Flavour) 6축 레이더·피니시 / 종합 / 히스토리). 등록 시 **Claude(sonnet-5)가 종류·증류소·도수·향/맛/피니시·aroma/flavour 6축 프로파일(0~4)·설명·평가를 자동 생성**(`src/lib/translate.ts whiskyInfo`). **감각 6축(2026-08-21, 기존 8축에서 축소)**: 스카치 플레이버 휠 소비자용 육각형 = `cereal/fruity/floral/peaty/woody/winey`, 라벨은 `영문명(한글명)`(**Cereal/Malty**(곡물)·Fruity(과일)·Floral(꽃향)·**Peaty/Smoky**(피트)·**Woody/Oaky**(우디)·**Winey/Sherry**(와인) — 동의어 병기, Fruity·Floral만 단독). 생산자 QC용 기술축 **feinty(페인티)·sulphury(유황)는 제거**(밸런스 블렌드에서 대부분 0→도형 왜곡). `Radar` 타입·`toRadar`·AI 프롬프트·`WhiskyRadar.tsx AXES`·`page.tsx RAXES` 모두 6키. 기존 jsonb에 남은 feinty/sulphur 키는 무시(마이그레이션 불필요). 레이더 축 라벨은 SVG 2줄(영문/한글). 필드 편집 저장 + `🔄 프로필 재생성`(PATCH `/api/whisky/[id]` GET/PATCH). 레이더는 무의존 SVG(`components/WhiskyRadar.tsx`). 컬럼: `type,distillery,abv,nose,palate,finish,aroma,flavour,description,evaluation`(자동) + `color,rating`(개인). ※`whisky_profile.personal_note`는 히스토리로 이관(아래) 후 미사용. **`tasted_on`(시음일)은 2026-07-25 상세 헤더 입력칸 제거**(히스토리 일자로 대체, 컬럼은 잔존하나 UI 미노출).
...
- **드릴다운**: 목록에서 주류명 클릭 → 그 술의 **① 가격 추이 그래프**(무의존 SVG 라인차트 `PriceTrendChart`, x=기준일자·y=가격·판매점별 색상 라인+점·범례) + **② 기준일자 × 판매점 가격표**(HTML 피벗, 최저가 셀 강조) + 최저/평균/최고 + 출처링크. 상·하단에 "← 주류시세 목록" 복귀 버튼. 홈 카드 🏷️ 주류시세 · Sidebar `🏷️ 주류시세`.
  ...
  - **시세 삭제/수동가격 + 데일리샷 품목ID 키(2026-08-19)**: 시세는 시트/데일리샷 동기화로 덮어써지므로 **동기화에 안전한 레이어**로 처리. (초안이던 별칭 병합/이름편집(`price_alias`)은 **삭제로 대체·제거**, 마이그레이션 `20260819110000_dailyshot_appid_hide.sql`에서 drop.)
    ...
    - **노트 상세 시세=매칭시세 읽기전용(2026-08-21)**: 노트 상세(`/whisky/[id]`)의 **가격 최저/평균/최고는 매칭된 시세**(price_name→없으면 name_ko/name_en, nkey 일치)에서 계산해 표시만. 노트 자체의 **`＋시세` 수동관측 추가 폼·관측 칩 UI 제거**(시세 데이터는 시세 화면에서만 편집 = 단일 출처). 신설 `GET /api/prices/by-name?name=` → `getPricesFromDB()`를 nkey(공백제거) 일치로 필터해 `{prices:[{price,date,shop,source}]}` 반환(시트/데일리샷/수동 union). 페이지는 `[price_name,name_ko,name_en]` 변경 시 fetch→`sise` state→siseMin/siseMax/avg 계산, 매칭 없으면 "위 시세매칭에서 연결" 안내 + `시세 N건 기준 (연결: …)`·`시세 화면` 링크(`/prices?name=`). 폐기 심볼: `Obs`타입·`obs`/`oForm` state·`addObs`/`delObs`. (기존 `hobby.price_observation` 노트별 관측 테이블은 UI 미노출.)
