...
- **드릴다운**: 목록에서 주류명 클릭 → 그 술의 **① 가격 추이 그래프**(무의존 SVG 라인차트 `PriceTrendChart`, x=기준일자·y=가격·판매점별 색상 라인+점·범례) + **② 기준일자 × 판매점 가격표**(HTML 피벗, 최저가 셀 강조) + 최저/평균/최고 + 출처링크. 상·하단에 "← 주류시세 목록" 복귀 버튼. 홈 카드 🏷️ 주류시세 · Sidebar `🏷️ 주류시세`.
  ...
  - **시세 병합/이름편집/수동가격(2026-08-19, 마이그레이션 `20260819100000_price_merge_manual.sql`)**: 시세는 시트/데일리샷 동기화로 덮어써지므로, **동기화에 안전한 별도 레이어** 2개 도입 — ① `hobby.price_alias(from_name PK, to_name)` = 이름 별칭(병합·개명), ② `hobby.liquor_price_manual` = 수동 판매점 시세(일자·판매점·가격, 동기화 미터치). **읽기 경로 `getPricesFromDB`가 liquor_price+manual union 후 별칭 전이해석(A→B→C)** 적용해 반환(`PriceRow`에 `id`·`source` 추가). API: `POST/DELETE /api/prices/alias`(from 단일/배열→to), `POST/DELETE /api/prices/manual`. UI(`prices/page.tsx`): **목록 행 체크박스 + 2종↑ 선택 시 병합 바**(유지할 이름 select→`병합`=나머지를 별칭). **드릴다운**: 이름 편집 input+`이름 저장`(=selected→새이름 별칭), **`＋ 판매점 시세 수동 입력`**(일자/판매점/가격→manual, 추가된 것은 칩으로 표시·✕삭제). 별칭은 되돌리기 가능(alias DELETE). 예: [조니워커 블랙 라벨]+[조니워커 블랙 700ml] 체크→병합→1개.
